...
const base_url_api = `${import.meta.env.VITE_REACT_UNICODE_API_URL}`;
...
*/
function analyzeCodePointByteArrayUtf16(
  byteArray: Buffer,
  codePoints: number
): Utf16Addup[] {
  // How Many Values
  ...
  if (codePoints == 1) {
    return [
      {
        two_byte_hex:
          byteArray.slice(1, 2).toString('hex').padStart(4, '0') +
          byteArray.slice(0, 1).toString('hex').padStart(4, '0'),
        two_byte_bin:
          byteArray[1].toString(2).padStart(8, '0') +
          byteArray[0].toString(2).padStart(8, '0'),
    ...
    const first_surrogate_value =
      (0x3 & byteArray[1] & 0x3ff) * 0x100 + byteArray[0];
    ...
    return [
      {
        two_byte_hex:
          byteArray.slice(1, 2).toString('hex').padStart(2, '0') +
          byteArray.slice(0, 1).toString('hex').padStart(2, '0'),
        two_byte_bin:
          byteArray[1].toString(2).padStart(8, '0') +
          byteArray[0].toString(2).padStart(8, '0'),
      ...
      {
        two_byte_hex:
          byteArray.slice(3, 4).toString('hex').padStart(2, '0') +
          byteArray.slice(2, 3).toString('hex').padStart(2, '0'),
        two_byte_bin:
          byteArray[3].toString(2).padStart(8, '0') +
          byteArray[2].toString(2).padStart(8, '0'),
        surrogate_mask: 'xxxxxx0000000000',
        value: second_surrogate_value,
        multiplier: 1,
        result: second_surrogate_value.toString(10),
        accumulation_hex:
          (first_surrogate_value * 0x400 + second_surrogate_value).toString(
            16
          ) +
          ' + 0x10000 = ' +
          (
            first_surrogate_value * 0x400 +
            second_surrogate_value +
            0x10000
          ).toString(16),
        accumulation_dec:
          (first_surrogate_value * 0x400 + second_surrogate_value).toString(
  ...
  for (let i = 0; i < buffer.length; ) {
    ...
    if (firstCodePointValue >= 0xd800 && firstCodePointValue <= 0xdbff) {
      ...
      if (!(secondByteValue >= 0xdc00 && secondByteValue <= 0xdfff)) {
        console.error(
          `Invalid UTF-16 sequence at position ${i}, not 2nd part of pair`
        );
        break;
      }
    }
    ...
    for (let j = 0; j < numTwoBytes; j++) {
      NewHexRepresentation += newBuffer
        .slice(j * 2, j * 2 + 2)
        .reverse()
        .toString('hex')
        .padStart(2 * numTwoBytes, '0');
    }
    ...
    examinations.push({
      ...
      addUps: analyzeCodePointByteArrayUtf16(
        buffer.slice(i, i + 2 * numTwoBytes),
        numTwoBytes
      ),
    });
